...
# eqtl_normial = "ciri2_qtl_mapping_nominal/nominal_pass_all_chrom_QTLtools.tsv.gz"
# eqtl_permute = "ciri2_qtl_mapping_apply_qvalue/ciri2.tsv"
# gwas_sumstat = "/sigma4/projects/rnaQTL_hg38/data/sumstat/T1D.tsv.gz"
# meta = "data/meta_sumstat.csv"
...
#     if(nrow(input) > 0){
#     dataset1=list(pvalues=input$p_value, type="cc", s = case_prop, N=nrow(gwas))
#     dataset2=list(pvalues=input$V7, type="quant", N=nrow(eqtl))
#     result <- coloc.abf( dataset1, dataset2, MAF=input$effect_allele_frequency)
#     #qtl_hit$PP.H3.abf[i] = result$summary[5]
#     qtl_hit$PP.H4.abf[i] = result$summary[6]
#     }
